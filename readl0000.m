%Version L: This version will be used to study the stratified flow cases for Richardson numbers from 0.1 to 10
%
%**************************************************************************
...
%Reading in the Data
%uvelfileID = fopen('corr2pt3dq0000.cbin','rb');
uvelfileID = fopen('corr2pt3dwm0000.cbin','rb');
...
%range: ~0.048min (not very good...) -- ~0.89max (also pretty bad)
searchvalue = 0.3
...
ylabel('Y Axis')
zlabel('Z Axis')
title('Elipsoid Fit at Ri = 0.0')
ax = gca;
ax.XTick = [-pi/8 0 pi/8];
ax.YTick = [-pi/8 0 pi/8];
ax.ZTick = [-pi/8 0 pi/8];
ax.XLim = [-pi/8 pi/8];
ax.YLim = [-pi/8 pi/8];
ax.ZLim = [-pi/8 pi/8];
ax.XTickLabel = {'-\pi/8','0','\pi/8'};
ax.YTickLabel = {'-\pi/8','0','\pi/8'};
ax.ZTickLabel = {'-\pi/8','0','\pi/8'};
grid on;
hold on;
...
radii2 = [radii(1), radii(2), radii(3)];
maxd = max( radii2 );
step = maxd / 63;
[ x, y, z ] = meshgrid( -maxd:step:(maxd + step) , -maxd:step:(maxd + step) , -maxd:step:(maxd + step) );
...
step = 2*pi/127;
[ yy, xx, zz ] = meshgrid( (-pi+step):step:(pi+step) , (-pi+step):step:(pi+step) , (-pi+step):step:(pi+step) );
p = patch(isosurface(xx,yy,zz,(uve),searchvalue));
p.FaceColor = 'red';
p.EdgeColor = 'none';
alpha(p,alphas);
...
ang = atand(eva(2)/eva(1))%get the angle inv tan of z/x (~0) & y/x (y/x is most important)
...
legend('Ellipsoid Fit','Isosurface','Semi-Major Axis','Semi-Minor Axis','Semi-Minor Axis');
